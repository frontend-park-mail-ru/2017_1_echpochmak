...
class SingleStrategy {
	...
	generateTower(field) {
		...
		field['field'].removeEventListener('mousedown', this.onClickField);
		circle.draw.addEventListener('mousedown', () => { this.createVariants(field) } );
	...
	createVariants(field) {
		...
		if (currentNewTower){
			...
			variantStay.draw.addEventListener('mousedown', () => {this.onClickStayVariant(field, currentNewTower)});
	...
	playerStep() {
		if (this.newStones >= 5){
			for (let i = 0; i < __WEBPACK_IMPORTED_MODULE_0__settings_js__["a" /* default */].mapSize; i++){
				for (let j = 0; j < __WEBPACK_IMPORTED_MODULE_0__settings_js__["a" /* default */].mapSize; j++){
					fields[i][j]['field'].removeEventListener('mousedown', this.onClickField);
				}
			}
			let waveButton = new variantBlock(4);
			waveButton.draw.addEventListener('mousedown', this.onClickWaveButton)
